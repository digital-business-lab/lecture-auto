# Block 4 Assignment — AGV Simulation in Shaft Manufacturing
...
After building your model and running all scenarios:

1. Compile your data into a results table
2. Create charts showing throughput, lead time, and WIP comparisons
3. Write your business report with recommendations on optimal AGV deployment
...
- **Plant Simulation User Guide** (available via institutional licence)
  - Chapter: Creating Models from Scratch
  - Chapter: Transport Systems and AGVs
  
- **Video Tutorials** (check Moodle for links)
  - Getting Started with Plant Simulation
  - Building Your First Production Line

- **Office Hours** (check schedule on Moodle)

---

*Remember: Start early! Building these models from scratch takes time, especially if you're new to Plant Simulation.*
...
- Build a discrete event simulation model in Plant Simulation from scratch (no coding required)
- Configure AGV transport systems with different parameters
- Run multiple simulation scenarios and collect statistical data
- Analyze how AGV count, transport times, and production volumes affect throughput, lead time, WIP, and costs
- Translate simulation results into business recommendations

---

## Scenario Description

A manufacturing company produces shafts for automotive applications. The production process consists of the following stations:

| Station | Function | Processing Time (min) |
|---|---|---|
| Raw Material Storage | Input buffer for raw shaft blanks | — |
| CNC Machining | Primary shaping of shafts | 8–12 (random) |
| Quality Inspection | Dimensional checks and testing | 3–5 (random) |
| Packaging | Final packaging for shipment | 2–4 (random) |
| Finished Goods Storage | Output buffer for completed shafts | — |

### AGV-Supported Layout

- Automated Guided Vehicles (AGVs) handle internal transport between stations
- Each station has an input/output buffer for AGV interaction
- AGVs follow predefined paths between stations
- Students will experiment with different numbers of AGVs and transport parameters
